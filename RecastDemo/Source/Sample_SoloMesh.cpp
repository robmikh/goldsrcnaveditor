...
void Sample_SoloMesh::handleDebugMode()
{
	...
	if (m_geom)
	{
		dtNavMesh* SelectedMesh = m_NavMeshArray[m_SelectedNavMeshIndex].m_navMesh;
		dtNavMeshQuery* SelectedQuery = m_NavMeshArray[m_SelectedNavMeshIndex].m_navQuery;

		valid[DRAWMODE_NAVMESH] = SelectedMesh != 0;
		valid[DRAWMODE_NAVMESH_TRANS] = SelectedMesh != 0;
		valid[DRAWMODE_NAVMESH_BVTREE] = SelectedMesh != 0;
		valid[DRAWMODE_NAVMESH_NODES] = SelectedQuery != 0;
		valid[DRAWMODE_NAVMESH_INVIS] = SelectedMesh != 0;
...
void Sample_SoloMesh::handleRender()
{
		...
		m_drawMode == DRAWMODE_NAVMESH_INVIS))
	{
		if (m_drawMode != DRAWMODE_NAVMESH_INVIS)
			duDebugDrawNavMeshWithClosedList(&m_dd, *CurrentMesh, *CurrentQuery, m_navMeshDrawFlags);
		if (m_drawMode == DRAWMODE_NAVMESH_BVTREE)
			duDebugDrawNavMeshBVTree(&m_dd, *CurrentMesh);
...
bool Sample_SoloMesh::handleBuild()
{
	...
	const int* surfTypes = m_geom->getMesh()->getSurfaceTypes();
	...
	for (auto it = AllMeshes.begin(); it != AllMeshes.end(); it++)
	{
		...
		m_cfg.walkableSlopeAngle = it->MaxSlope;
		m_cfg.walkableHeight = (int)ceilf(it->AgentStandingHeight / m_cfg.ch);
		m_cfg.crouchHeight = (int)ceilf(it->AgentCrouchingHeight / m_cfg.ch);
		m_cfg.walkableClimb = (int)floorf(it->MaxStep / m_cfg.ch);
		m_cfg.walkableRadius = (int)ceilf(it->AgentRadius / m_cfg.cs);
		...
		rcMarkWalkableTriangles(m_ctx, m_cfg.walkableSlopeAngle, verts, nverts, tris, ntris, m_triareas, surfTypes);
		...
		if (m_filterWalkableLowHeightSpans)
			rcFilterWalkableLowHeightSpans(m_ctx, m_cfg.walkableHeight, m_cfg.crouchHeight, *m_solid);
		...
		if (!rcBuildCompactHeightfield(m_ctx, m_cfg.crouchHeight, m_cfg.walkableClimb, *m_solid, *m_chf))
		{
		...
		if (m_cfg.maxVertsPerPoly <= DT_VERTS_PER_POLYGON)
		{
			...
			params.NumOffMeshConnections = m_geom->getOffMeshConnectionCount();
			params.walkableHeight = it->AgentStandingHeight;
			params.walkableRadius = it->AgentRadius;
			params.walkableClimb = it->MaxStep;
			...
			m_NavMeshArray[MeshIndex].m_navMesh = dtAllocNavMesh();
			if (!m_NavMeshArray[MeshIndex].m_navMesh)
			{
				dtFree(navData);
			...
			status = m_NavMeshArray[MeshIndex].m_navMesh->init(navData, navDataSize, DT_TILE_FREE_DATA);
			...
			status = m_NavMeshArray[MeshIndex].m_navQuery->init(m_NavMeshArray[MeshIndex].m_navMesh, 2048);
			if (dtStatusFailed(status))
			{
